...
# Shared functions
def get_filtered_entities_for_room(hass, room_id, entity_type=None):
    """Retrieve the filtered entities for a specific room.
    
    If entity_type is provided (e.g. 'power' or 'energy'), only return sensor entity IDs
    that both start with room_id and end with either f"_{entity_type}" or f"_untracked_{entity_type}".
    """
    ...
    if entity_type:
        filtered_entities = [
            entity for entity in all_sensors
            if entity.startswith(room_id) and 
            (entity.endswith(f"_{entity_type}") or entity.endswith(f"_untracked_{entity_type}"))
        ]
    else:
...
def get_selected_entities_for_rooms(hass, selected_existing_rooms, integration_entities, selected_entities, entity_type):
    ...
    for room_id in selected_existing_rooms:
        if room_id in integration_entities:
            ...
            # (Note: room_id here is assumed to be the full sensor entity ID)
            room_entities = get_filtered_entities_for_room(hass, room_id, entity_type)
            if room_entities:
...
class EnergyandPowerMonitorOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_user(self, user_input=None):
        ...
        errors = {}
        TRANSLATION_NONE = await get_translated_none(self.hass)
        old_room = self._config_entry.data.get(CONF_ROOM, "")
        old_entities_smd = self._config_entry.data.get(CONF_SMART_METER_DEVICE, "")
        old_entities = set(self._config_entry.data.get(CONF_ENTITIES, []))
        old_integration_rooms = self._config_entry.data.get(CONF_INTEGRATION_ROOMS, [])
        current_room = self._config_entry.data.get(CONF_ROOM, "")
        ...
        if user_input is not None:
            try:
                if user_input[CONF_ROOM] != old_room:
                    current_entity_type = self._config_entry.data.get(CONF_ENTITY_TYPE)
